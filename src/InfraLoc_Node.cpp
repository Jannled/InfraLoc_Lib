...
int InfraNode::publishBucketStrength(std::array<number_t, INFRALOC_NUM_CHANNELS> values, const number_t angle, const float freq)
{
...
{
	volatile number_t alpha = SMALLER_ANGLE(angle_b - angle_c);
	volatile number_t beta = SMALLER_ANGLE(angle_c - angle_a);
	volatile number_t gamma = SMALLER_ANGLE(angle_a - angle_b);
